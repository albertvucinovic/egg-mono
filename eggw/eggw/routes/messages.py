...
def _get_messages_sync(
    ...
    normalization so large web transcripts can show the recent tail quickly.
    ``create_snapshot`` is used instead of rebuilding from every event on every
    request; it incrementally catches up the cached snapshot when possible and
    only falls back to a full scan for non-incremental mutations.
    """
    ...
    try:
        ...
        snap_raw = create_snapshot(fresh_db, thread_id)
