...
- The published tool result for this special tool should be `no_api=True` and `keep_user_turn=True` so it does not pollute provider context or trigger an extra LLM turn by itself.
...
- [x] Ensure published tool result is hidden from provider context and does not trigger RA1 by adding `no_api=True` and `keep_user_turn=True` for this tool's tool message.
...
- [x] Add focused eggthreads tests:
  ...
  - provider context sent after the tool result excludes the interim answer and hidden tool result.
