...
- [x] Phase 5 — Integration polish
  ...
  - Status notes:
    - 2026-05-10: Added focused renderer regressions for source replacement preserving appended in-session rows at the bottom and for `stream_end()` followed by finalized `print_above()` leaving no stale stream rows or duplicate final rows after a later source refresh.
    - 2026-05-10: Added egg integration regressions proving full-screen new-message/finalized-message paths still append locally via `console_print_message()`/`print_above`, local rows remain visible until redraw/source replacement, redraw clears local rows before replacing the source, and stream-close batches finalize exactly once after `stream_end()` without replacing the source in-session. No production fixes were needed. Test runs: `python -m pytest eggdisplay/tests/test_renderers_terminal_safety.py egg/tests/test_panels.py::TestFullScreenScrollbackWiring egg/tests/test_streaming_tui.py -q` (31 passed), `python -m pytest eggdisplay/tests -q` (47 passed), `python -m pytest egg/tests/test_commands_display.py egg/tests/test_commands_thread.py egg/tests/test_panels.py egg/tests/test_streaming_tui.py -q` (123 passed), `python -m pytest egg/tests -q` (400 passed).
