...
def test_attachment_tools_are_registered_with_schema_and_help() -> None:
    ...
    path_description = specs["add_local_file_to_model_context"]["parameters"][
        "properties"
    ]["path"]["description"]
    assert "/workspace/image.png" in path_description
    export_path_description = specs["save_provider_artifact_to_file"]["parameters"][
        "properties"
    ]["path"]["description"]
    assert "/workspace/output.png" in export_path_description
...
def test_add_provider_artifact_to_model_context_uses_db_storage_with_changed_cwd(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    ...
    part = json.loads(output)["content_part"]
    _metadata, data = resolve_input_bytes(run, db, tid, part["input_id"])

    assert data == b"generated bytes"
    assert not (working_dir / ".egg" / "egg_inputs").exists()
...
def test_save_provider_artifact_to_file_maps_docker_visible_absolute_path(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    db = _make_db(tmp_path)
    tid = ts.create_root_thread(db, name="root")
    ts.set_thread_working_directory(db, tid, "work")
    ...
    output = ts.create_default_tools().execute(
        "save_provider_artifact_to_file",
        {"artifact_id": source.artifact_id, "path": "/workspace/exports/out.txt"},
        db=db,
        thread_id=tid,
    )
    payload = json.loads(output)
...
def test_save_provider_artifact_to_file_maps_docker_visible_directory(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    db = _make_db(tmp_path)
    tid = ts.create_root_thread(db, name="root")
    ts.set_thread_working_directory(db, tid, "work")
    (tmp_path / "work" / "exports").mkdir(parents=True)
    source = save_provider_output_bytes(
        tmp_path,
        tid,
        b"generated bytes",
    ...
    output = ts.create_default_tools().execute(
        "save_provider_artifact_to_file",
        {"artifact_id": source.artifact_id, "path": "/workspace/exports"},
        db=db,
        thread_id=tid,
    )
    payload = json.loads(output)

    assert payload["path"] == "exports/generated.txt"
    assert (tmp_path / "work" / "exports" / "generated.txt").read_bytes() == b"generated bytes"
...
def test_scheduler_resolves_docker_visible_absolute_image_path(tmp_path, monkeypatch):
    import asyncio

    monkeypatch.chdir(tmp_path)
    db = _make_db(tmp_path)
    tid = ts.create_root_thread(db, name="Actor")
    working_dir = tmp_path / "actor"
    ts.set_thread_working_directory(db, tid, working_dir)
    image = working_dir / "scratch" / "current-grid.png"
    image.parent.mkdir(parents=True)
    image.write_bytes(b"\x89PNG\r\n\x1a\nimage-bytes")
    tool_call_id = "call-add-container-image"
    ts.append_message(
        db,
        tid,
        "assistant",
            ...
            "tool_calls": [
                {
                    ...
                    "function": {
                        "name": "add_local_file_to_model_context",
                        "arguments": json.dumps(
                            {"path": "/workspace/scratch/current-grid.png"}
                        ),
                    },
                }
            ]
    ...
    runner = ts.ThreadRunner(db, tid, llm=object(), tools=ts.create_default_tools())
    assert asyncio.run(runner.run_once()) is True
    assert asyncio.run(runner.run_once()) is True
    messages = ts.create_snapshot(db, tid)["messages"]
    tool_message = next(message for message in messages if message.get("role") == "tool")
    attachment = next(
        part for part in tool_message["content"] if part.get("type") == "attachment"
    )
    assert attachment["filename"] == "current-grid.png"
    assert attachment["presentation"] == "image"
