...
BUILTIN_TOOL_HELP_DETAILS: dict[str, dict[str, Any]] = {
    ...
    "generate_image": {
        ...
        "notes": [
            ...
            "Backends using codex_images generate one image per call; omit n or use n=1 for them.",
            "ChatGPT/Codex OAuth subscription image generation uses the Codex-style images/generations endpoint, not a forced Responses image_generation tool call.",
            "Use `save_provider_artifact_to_file` when the user needs export, or `add_provider_artifact_to_model_context` when a generated artifact should be reused as model input.",
        ],
        "examples": [
            '{"prompt": "A watercolor egg-shaped spaceship in a pine forest"}',
            '{"model": "OpenAI Pro Image: gpt-image-2", "prompt": "A cinematic neon cat portrait", "quality": "high", "size": "1024x1024"}',
    ...
    "add_local_file_to_model_context": {
        ...
        "notes": [
            ...
            "For Docker-sandboxed threads, absolute paths visible inside the container (normally under `/workspace`, or an explicit extra mount) are translated to their host mount source.",
    ...
    "add_provider_artifact_to_model_context": {
        "details": "Promote an accessible provider-output artifact into input attachment storage and add it to the current thread's model context, using the same shared operation as `/attachOutput`.",
        "use_when": [
            "A generated/provider artifact should be reused as a model input in a later model call.",
            "The user asks you to inspect, compare, or reason over an image/file artifact produced by `generate_image` or another provider-output flow.",
        ],
        "notes": [
            "If the artifact is an image, Egg sends it as visual input to the next image-capable provider call; this is the right tool when the LLM needs to actually see a generated image.",
            ...
            "The result includes attachment content parts and safe metadata, not raw storage paths, bytes, or base64.",
    ...
    "save_provider_artifact_to_file": {
        "details": "Save an accessible provider-output artifact to a user-visible file under the current thread working directory, using the same shared operation as `/saveProviderArtifact`.",
        "use_when": [
        ...
        "notes": [
            ...
            "This tool saves a file for the user/project; it does not add the artifact to model context. Use `add_provider_artifact_to_model_context` when the model needs to inspect the artifact.",
            ...
            "For Docker-sandboxed threads, absolute paths visible inside the container (normally under `/workspace`, or an explicit extra mount) are translated to their host mount target before those checks.",
            "Omit `path` to use the artifact's safe filename; pass a directory or filename to control the export location.",
            "The same provider-output access rules as `add_provider_artifact_to_model_context` apply, including explicit `descendant_thread_id` for ancestor-to-descendant reads.",
        ],
        "examples": [
            '{"artifact_id": "abc12345"}',
...
def _generate_image_dynamic_help(*, raw_context: Mapping[str, Any]) -> list[str]:
    ...
    for key, cfg_raw in models_config.items():
        ...
        if api_type:
            ...
            if str(api_type).strip().lower().replace("-", "_") == "codex_images":
                lines.append("    option note: one image per call; omit `n` or use `n=1`")
                lines.append("    endpoint note: posts to `images/generations` relative to the provider base URL")
        if model_name:
