...
chosen actions. Plain `python plan.py` runs A* by default. Define a matching
`goal_<suffix>` and, when useful, `heuristic_<suffix>` so that default is
productive; explicitly choose reward or combined search when appropriate. At a
high level, matching `reward_<suffix>` functions enable bounded breadth-first
progress search, while matching `goal_<suffix>` and optional
`heuristic_<suffix>` functions enable A* goal search. Run
...
3. Inspect and revise `world_model.py`; preserve genuinely plausible competing
   hypotheses and define matching `goal_<suffix>` functions (plus useful
   `heuristic_<suffix>` functions when possible) so the default A* planner can
   search them. Add `reward_<suffix>` when reward search is also useful.
4. Run `python backtest.py` and resolve relevant Timeline mismatches.
5. Run `python plan.py` to perform the default A* search over surviving
   hypotheses. Inspect
