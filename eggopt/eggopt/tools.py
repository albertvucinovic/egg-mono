...
def safe_tools(
    ...
    *,
    ...
    """Return a registry and its explicit least-privilege capability set."""
    ...
    allowed = SAFE_TOOLS if allowed_tools is None else frozenset(allowed_tools)
    available = {item["function"]["name"] for item in registry.tools_spec()}
    missing = allowed - available
    if missing:
        raise ValueError(f"tool registry is missing allowed tools: {sorted(missing)}")
    return registry, allowed


def default_safe_tools() -> ToolRegistry:
    return safe_tools()[0]


__all__ = ["SAFE_TOOLS", "default_safe_tools", "safe_tools"]
