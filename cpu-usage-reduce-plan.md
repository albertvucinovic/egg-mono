...
- Status: idle token stats are now snapshot-watermark driven instead of TTL-driven, avoiding periodic multi-MB `snapshot_json` parsing while the snapshot is unchanged.
- Last updated: after idle token-stats TTL removal.
- Recommended next action: reload current Egg and re-profile. If CPU remains high, share the snapshot watermark across one panel update tick and/or investigate Rich rendering. Also restart/re-profile the long-running `eggw` Hypercorn worker if web CPU matters.
