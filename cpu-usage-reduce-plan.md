...
- Status: `/reload` now refuses to reload while the current thread or any descendant has an active stream lease, including tool streams.
- Last updated: after `/reload` streaming/subtree guard implementation.
- Recommended next action: push/reload idle Egg instances, then investigate the GitHub Actions failure at https://github.com/albertvucinovic/egg-mono/actions.
