...
- Status: post-reload profiling after the first token/TPS fix found the remaining idle hot path: periodic `current_token_stats()` cache misses still call `thread_token_stats()` and parse multi-MB `snapshot_json`; smaller repeated snapshot-watermark SQL probes and Rich rendering are secondary. One web/Hypercorn worker also still spends CPU in scheduler max-event polling and should be restarted/profiled separately.
- Last updated: after post-`4ef51a3` reload py-spy profiling.
- Recommended next action: make idle token stats event-watermark driven instead of TTL-driven, and share the snapshot watermark across the panel update tick so it is not queried separately by chat cache, token stats, and TPS header. Also restart/re-profile the long-running `eggw` Hypercorn worker if web CPU matters.
