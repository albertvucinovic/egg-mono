...
- Status: Phase 4.6 scheduler max-event bulk query fix completed after `py-spy` showed a hot active TUI spending ~58% of samples in scheduler polling, mostly `_max_event_seqs_bulk()`.
- Last updated: after reloading most hot Egg TUI instances with Phase 4.6 code.
- Recommended next action: if CPU remains high after the current controlling Egg is manually reloaded, inspect token-stat/TUI panel samples next; do not rely on long-lived `egg.sh` wrapper `%CPU` as current CPU.
