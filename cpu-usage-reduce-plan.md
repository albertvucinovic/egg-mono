...
- Last updated: after Phase 4.6 scheduler max-event bulk query fix.
- Recommended next action: restart/reload the hot Egg TUI instances, then re-profile. If CPU remains high, inspect token-stat/TUI panel samples next; do not rely on long-lived `egg.sh` wrapper `%CPU` as current CPU.
