...
- 2026-07-03: Follow-up observability added `/cost` reporting for local optimizer publication savings: optimized tool-output count, raw vs published chars, approximate token savings, per-filter breakdown, and a note that actual billing still depends on later API calls, compaction, caching, and model pricing.
- 2026-07-03: Default-on/bash follow-up implemented. The native optimizer is now enabled by default unless `EGG_OUTPUT_OPTIMIZER=off`/false-like config disables it, unchanged outputs stay default-policy identical, visible `$` user-command outputs continue through the same publication optimizer, optimized outputs publish a raw-artifact recovery note and stash an artifact when needed, `ls -l`/`ls -la` long listings are summarized, grep/path-list output-shape filters cover complex bash wrappers, and the generic fallback can bound medium bash output while preserving raw recovery. A quick inspection of this long thread showed large `stdout` bash outputs dominate published characters, so the generic bounded fallback is the highest-impact safety net.
- 2026-07-04: `/cost` optimizer reporting now emphasizes the operational ratios users need: optimized tool calls vs total tool calls, total tool-output context baseline vs published output and savings percentage across all tool output (not just triggered calls), plus an approximate full-thread context saved estimate. Per-filter breakdown remains for diagnosis.
