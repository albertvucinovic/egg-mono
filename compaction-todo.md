...
- 2026-05-10: Bug fix for summary compaction tool-call boundary. `compact_thread()` now passes its running tool call id and parent assistant message id into the compaction core, allowing the current `compact_thread` tool call's own pending tool result to satisfy provider tool-call protocol validation. The model-visible tool result is now the concise normal result `Thread compacted.` on success, so `/compactWithSummary` can compact after an assistant summary that calls `compact_thread()` without being rejected for its own not-yet-published tool result.
  - Changed files: `eggthreads/eggthreads/api.py`, `eggthreads/eggthreads/builtin_plugins/compaction.py`, `eggthreads/tests/test_compaction.py`.
  - Tests: `pytest -q eggthreads/tests/test_compaction.py::test_compact_thread_tool_is_registered_and_emits_event eggthreads/tests/test_compaction.py::test_compact_thread_tool_allows_current_tool_call_parent_as_default_start eggthreads/tests/test_compaction.py::test_compaction_rejects_assistant_tool_call_start_without_complete_results` passed; `pytest -q eggthreads/tests/test_compaction.py eggthreads/tests/test_command_registry.py eggthreads/tests/test_plugin_tool_registry.py` passed; `python -m compileall -q eggthreads/eggthreads` passed.
