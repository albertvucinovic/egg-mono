...
- [x] Ensure `update_panels()` never calls expensive token/reducer functions more often than necessary during active streams.
- [x] Prefer cached/stale token metrics while streaming over blocking input/scroll.
- [x] Avoid duplicate header TPS/token metric calls in the same panel update.
- [x] Preserve visible metrics when idle or after stream completion.
