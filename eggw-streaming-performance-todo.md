...
- 2026-07-13 — Scroll follow-up after manager edge review. Upward wheel/touch/key intent now creates one fenced pending boundary token: `onScroll` may service the 240 px threshold only while that explicit token exists, and one input-owned RAF handles the already-clamped/no-scroll case; whichever path wins clears/cancels the token exactly once, while generic/programmatic scroll without a token remains inert. Home explicitly applies its native top destination before the shared causal check. Fetch operations now carry a monotonic operation identity, settle exactly once either through post-commit anchor restoration or an explicit no-render-change success path, and ignore stale settle callbacks after thread changes/newer work. This prevents authoritative empty/overlap pages from leaving `fetching`/`isLoadingOlder` stuck while preserving queued demand and normal anchor restoration. Deterministic Playwright adds above-threshold single-wheel and Home crossings plus an empty authoritative page with unchanged rendered start. Verification: full frontend unit suite (74 passed); TypeScript (passed); focused new/existing scroll and min-history Playwright (6 passed); full frontend Playwright suite (79 passed); `git diff --check` (passed). Exact next task: none for this follow-up.

- 2026-07-13 — Final causal-attribution correction: removed the fixed three-frame boundary window. One explicit upward input now creates one pending token; its causally resulting `scroll` event services threshold crossing, while one RAF is only the no-scroll fallback for an already-clamped input. Service, replacement input, or thread change fences and clears it exactly once. Focused Playwright also proves a generic programmatic top scroll without an input token does not reveal/fetch. Verification: full frontend unit suite (74 passed); TypeScript (passed); focused new/existing scroll and min-history Playwright (6 passed); full frontend Playwright suite (79 passed); `git diff --check` (passed). Exact next task: none.
