...
- 2026-07-13 — Scroll follow-up after manager edge review. Upward wheel/touch/key intent now owns one token-fenced, coalesced post-input RAF boundary check, so a gesture beginning above 240 px and landing/clamping at the top advances history exactly once while generic/programmatic `scroll` events remain inert; Home explicitly applies its native top destination before the shared boundary check. Fetch operations now carry a monotonic operation identity, settle exactly once either through post-commit anchor restoration or an explicit no-render-change success path, and ignore stale settle callbacks after thread changes/newer work. This prevents authoritative empty/overlap pages from leaving `fetching`/`isLoadingOlder` stuck while preserving queued demand and normal anchor restoration. Deterministic Playwright adds above-threshold single-wheel and Home crossings plus an empty authoritative page with unchanged rendered start. Verification: full frontend unit suite (74 passed); TypeScript (passed); focused new/existing scroll and min-history Playwright (6 passed); full frontend Playwright suite (79 passed); `git diff --check` (passed). Exact next task: none for this follow-up.
