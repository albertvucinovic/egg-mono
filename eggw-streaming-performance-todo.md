...
Last updated: 2026-07-11 01:26 UTC
...
- 2026-07-11 01:26 UTC — Phase 6 and the implementation plan complete. Measurement first: the reusable production-build `npm run profile:transcript` harness serves deterministic 0/100/300 mixed Markdown/code/math/tool-output fixtures, reports payload bytes, mounted DOM nodes, input command round-trip p95, browser Event Timing, long tasks, and development React/flush counters, and supports CDP throttling with `EGGW_CPU_RATE=4`. The exact Phase-5 baseline was rebuilt in a detached worktree at `38d4903` with the same harness. Baseline results (normal / 4x CPU): 0 messages — 97/98 DOM nodes, input p95 1.8/7.3 ms, max Event Timing 40/40 ms; 100 messages — 13,525 DOM nodes, input p95 3.9/23.8 ms, max Event Timing 32/40 ms; 300 messages / 2,320,014 bytes — 40,627/40,625 DOM nodes, input p95 8.9/57.3 ms, max Event Timing 40/64 ms. This failed the 4x composer and >50 ms input gates, so windowing was required. Implementation: `lib/transcriptWindow.ts` keeps all React Query pages authoritative but mounts the newest 10 messages, preserves the mounted start across tail append, and expands loaded history in 60-message chunks; `ChatPanel.tsx` adds explicit expansion while retaining existing backend load-older/scroll anchoring/stick-to-bottom, command chronology, and live cards. Minimum verbosity expands back to the latest visible conversation boundary and aggregates hidden reasoning/tool details from the remaining unmounted prefix, so hidden execution summaries are not lost. Mounted content retains normal browser copy/search; unmounted loaded content requires expansion. Post-window production results (normal / 4x CPU): 0 messages — 97/98 DOM nodes, input p95 1.6/7.1 ms, max Event Timing 24/40 ms; 100 messages — 1,441 DOM nodes, input p95 1.8/7.2 ms, max Event Timing 32/32 ms; 300 messages / 2,320,014 bytes — 1,450/1,449 DOM nodes, input p95 1.7/6.6 ms, max Event Timing 40/32 ms. Thus the 300-message 4x input p95 improved from 57.3 to 6.6 ms and no measured input event exceeded 40 ms. `performanceInstrumentation.ts` provides deterministic development Profiler/imperative-flush counters; browser gates prove typing 200 characters with 300 loaded messages causes zero `ChatPanel`/static-transcript commits and a 1,100-delta burst causes zero static-transcript commits, no body-driven panel commit (only an independently scheduled elapsed-time tick is allowed), and one coalesced body flush per channel in the measured burst. Tool argument full bodies remain RAF/direct-DOM; a separate 100 ms `IntervalCoalescer` limits compact previews to <=10/sec and is unit-tested with 1,100 notifications over one second. `npm run test:regression` reuses the existing continuity/reconnect/command/draft/autocomplete scenarios plus both performance gates rather than duplicating fixtures. `eggw/README.md` now documents the active replay cursor, synchronous durable/live handoff, buffer ownership, bounded transcript rendering, and profiling commands. Caveat: browser `longtask` entries include framework hydration/browser work and measured maxima still exceed 50/100 ms (post-window 300: 102 ms normal, 455 ms at 4x in the recorded run), while attributed static-transcript Profiler duration and ready time improved sharply; the harness reports this limitation instead of asserting a flaky CI wall-time budget. Event Timing and input round-trip gates meet the requested interaction target. Verification: final frontend unit suite (58 passed); TypeScript (passed); production build (passed); integrated regression Playwright (9 passed); final full Playwright (43 passed); focused backend event/state/message suite (19 passed, 95 deselected, one Starlette warning); full `eggthreads/tests eggw/tests` with ambient EggW security variables cleared (1,130 passed, 1 skipped, two warnings). An initial full backend run inherited the operator shell's `EGGW_ALLOWED_ORIGINS`/token configuration and failed two security-origin tests (1,128 passed, 1 skipped); rerunning in the suite's expected clean environment passed completely. `git diff --check` passed. No backend behavior changed in Phase 6. `review-20260709.md` remains untouched and untracked. Remaining open items: browser-wide hydration long-task maxima are documented but not suitable as deterministic CI assertions; no further implementation phase is planned. Any additional optimization should begin with the checked-in production profile harness rather than adding more virtualization by assumption.
