        ...
        if not getattr(self, '_display_is_inline', False):
            ...
            if max_messages > 0 and len(msgs) > max_messages:
                ...
                self._live_print(Panel(
                    f"[dim]Showing last {max_messages} messages; {omitted} earlier messages remain in thread history. Use inline mode or /redraw for a full console transcript.[/dim]",
                    border_style='blue',
