...
@dataclass
class TaskSpec:
    # Use kw_only=True to prevent inheritance issues with default values vs positional arguments in subclasses
    attempt: int = field(default=0, kw_only=True)
    rerun_on_corruption: bool = field(default=True, kw_only=True)
    ...
    async def run(self) -> Any: raise NotImplementedError
...
class JobStore:
    ...
    def get(self, key): return self.conn.execute("SELECT * FROM jobs WHERE cache_key=?", (key,)).fetchone()
    def create(self, key, spec):
        try: self.conn.execute("INSERT INTO jobs (cache_key, spec_blob, status) VALUES (?, ?, ?)", (key, pickle.dumps(spec), "PENDING")); self.conn.commit()
        except sqlite3.IntegrityError: pass
    def update(self, key, status, result=None, external_id=None):
        if result: self.conn.execute("UPDATE jobs SET status=?, result_blob=?, updated_at=CURRENT_TIMESTAMP WHERE cache_key=?", (status, pickle.dumps(result), key))
        elif external_id: self.conn.execute("UPDATE jobs SET status=?, external_id=?, updated_at=CURRENT_TIMESTAMP WHERE cache_key=?", (status, external_id, key))
        else: self.conn.execute("UPDATE jobs SET status=?, updated_at=CURRENT_TIMESTAMP WHERE cache_key=?", (status, key))
        self.conn.commit()
...
class EggFlowExecutor:
    ...
    async def run(self, spec: Union[TaskSpec, List[TaskSpec]]) -> Union[Result, List[Result]]:
        if isinstance(spec, list): return await asyncio.gather(*(self._execute_task(s) for s in spec))
    ...
    async def _execute_task(self, spec: TaskSpec) -> Result:
        ...
        if row and row['status'] == "COMPLETED":
            try:
                ...
                # Corruption Detected
                if not spec.rerun_on_corruption:
                    return Result(error=f"Cache Data Corrupted: {str(e)}", metadata={"corrupt": True})
                # Else: pass (fall through to re-execution)
        ...
        try:
            # 3. Execute
            if isinstance(spec, CreateThread): res = await self._handle_create_thread(spec, key, row)
            elif isinstance(spec, ContinueThread): res = await self._handle_continue_thread(spec, key)
            elif isinstance(spec, ForkThread): res = await self._handle_fork_thread(spec, key)
            else: res = await self._handle_generic(spec, key)
            
            # 4. Success -> COMPLETED
            return res
            
        except Exception as e:
            # 5. Exception -> COMPLETED (but with error result)
            err_res = Result(error=str(e))
            self.store.update(key, "COMPLETED", result=err_res)
            return err_res
    ...
    async def _run_scheduler(self, tid):
        ...
        finally: task.cancel(); await asyncio.sleep(0)
    ...
    async def _collect_outputs(self, tid: str, filenames: List[str]) -> Dict[str, str]:
        ...
        if not EGGTHREADS_AVAILABLE: return {f: f"Mock content of {f}" for f in filenames}
    ...
    async def _handle_create_thread(self, spec: CreateThread, key: str, row) -> Result:
        if row and row['external_id']: tid = row['external_id']
        else:
    ...
    async def _handle_continue_thread(self, spec: ContinueThread, key: str) -> Result:
        ...
        if not EGGTHREADS_AVAILABLE:
            res = Result(value=f"Mock Reply to: {spec.content}", metadata={"thread_id": tid, "artifacts": await self._collect_outputs(tid, spec.output_files)})
            self.store.update(key, "COMPLETED", result=res)
            return res
        snap = json.loads(egg_api.create_snapshot(self.egg_db, tid))
        msgs = snap.get("messages", [])
