"""Compatibility shim for importing eggflow from the monorepo root."""

from importlib import import_module
from pathlib import Path

_INNER_PACKAGE_DIR = Path(__file__).resolve().parent / "eggflow"
if _INNER_PACKAGE_DIR.is_dir():
    _inner = str(_INNER_PACKAGE_DIR)
    if _inner not in __path__:
        __path__.append(_inner)

_pkg = import_module('.eggflow', __name__)
